<template>
  <div class="row">
    <div class="col-xs-12">
      <q-tabs
        v-model="tab"
        align="justify"
        no-caps
        outside-arrows
        mobile-arrows
        class="bg-white text-primary"
      >
        <q-tab name="lanches" label="Lanches" />
        <q-tab name="pizzas" label="Pizzas" />
        <q-tab name="bebidas" label="Bebidas" />
      </q-tabs>
      <q-tab-panels v-model="tab" animated>
        <q-tab-panel name="lanches">
          <q-card class="q-mb-sm">
            <div class="row">
              <div class="col-xs-12 col-md-2">
                <q-img
                  src="https://offloadmedia.feverup.com/saopaulosecreto.com/wp-content/uploads/2020/10/27043606/pexels-valeria-boltneva-1639557-scaled.jpg"
                  :ratio="4 / 3"
                />
              </div>
              <div class="col-xs-12 col-md-10">
                <q-card-section>
                  <div class="row">
                    <div class="col-xs-11">
                      <div class="text-h6">X-Man</div>
                      <div class="text-subtitle2">R$ 10,00</div>
                    </div>
                    <div class="col-xs-1">
                      <div class="row justify-center">
                        <span class="quantity-label">0</span>
                      </div>
                      <div class="row justify-center">
                        <q-btn
                          round
                          size="xs"
                          color="primary"
                          icon="chevron_left"
                          class="q-mr-sm"
                        />
                        <q-btn
                          round
                          size="xs"
                          color="primary"
                          icon="chevron_right"
                        />
                      </div>
                    </div>
                  </div>
                </q-card-section>
                <q-card-section>
                  Lorem ipsum dolor sit amet, consectetur adipiscing elit
                </q-card-section>
              </div>
            </div>
          </q-card>
          <q-card class="q-mb-sm">
            <div class="row">
              <div class="col-xs-12 col-md-2">
                <q-img
                  src="https://media-cdn.tripadvisor.com/media/photo-s/1a/01/95/8d/ze-colmeia.jpg"
                  :ratio="4 / 3"
                />
              </div>
              <div class="col-xs-12 col-md-10">
                <q-card-section>
                  <div class="row">
                    <div class="col-xs-11">
                      <div class="text-h6">X-Montro</div>
                      <div class="text-subtitle2">R$ 10,00</div>
                    </div>
                    <div class="col-xs-1">
                      <div class="row justify-center">
                        <span class="quantity-label">0</span>
                      </div>
                      <div class="row justify-center">
                        <q-btn
                          round
                          size="xs"
                          color="primary"
                          icon="chevron_left"
                          class="q-mr-sm"
                        />
                        <q-btn
                          round
                          size="xs"
                          color="primary"
                          icon="chevron_right"
                        />
                      </div>
                    </div>
                  </div>
                </q-card-section>
                <q-card-section>
                  Lorem ipsum dolor sit amet, consectetur adipiscing elit
                </q-card-section>
              </div>
            </div>
          </q-card>
        </q-tab-panel>

        <q-tab-panel name="pizzas">
          <q-card class="q-mb-sm">
            <div class="row">
              <div class="col-xs-12 col-md-2">
                <q-img
                  src="https://www.receiteria.com.br/wp-content/uploads/receitas-de-pizza-quatro-queijos-00.jpg"
                  :ratio="4 / 3"
                />
              </div>
              <div class="col-xs-12 col-md-10">
                <q-card-section>
                  <div class="row">
                    <div class="col-xs-11">
                      <div class="text-h6">Pizza Quatro Queijos</div>
                      <div class="text-subtitle2">R$ 50,00</div>
                    </div>
                    <div class="col-xs-1">
                      <div class="row justify-center">
                        <span class="quantity-label">0</span>
                      </div>
                      <div class="row justify-center">
                        <q-btn
                          round
                          size="xs"
                          color="primary"
                          icon="chevron_left"
                          class="q-mr-sm"
                        />
                        <q-btn
                          round
                          size="xs"
                          color="primary"
                          icon="chevron_right"
                        />
                      </div>
                    </div>
                  </div>
                </q-card-section>
                <q-card-section>
                  Lorem ipsum dolor sit amet, consectetur adipiscing elit
                </q-card-section>
              </div>
            </div>
          </q-card>
          <q-card class="q-mb-sm">
            <div class="row">
              <div class="col-xs-12 col-md-2">
                <q-img
                  src="https://brcommartinlabsapptiteimagens.s3.amazonaws.com/chef/4488/rJ7ZUHpv8.jpg"
                  :ratio="4 / 3"
                />
              </div>
              <div class="col-xs-12 col-md-10">
                <q-card-section>
                  <div class="row">
                    <div class="col-xs-11">
                      <div class="text-h6">Pizza Portuguesa</div>
                      <div class="text-subtitle2">R$ 50,00</div>
                    </div>
                    <div class="col-xs-1">
                      <div class="row justify-center">
                        <span class="quantity-label">0</span>
                      </div>
                      <div class="row justify-center">
                        <q-btn
                          round
                          size="xs"
                          color="primary"
                          icon="chevron_left"
                          class="q-mr-sm"
                        />
                        <q-btn
                          round
                          size="xs"
                          color="primary"
                          icon="chevron_right"
                        />
                      </div>
                    </div>
                  </div>
                </q-card-section>
                <q-card-section>
                  Lorem ipsum dolor sit amet, consectetur adipiscing elit
                </q-card-section>
              </div>
            </div>
          </q-card>
        </q-tab-panel>

        <q-tab-panel name="bebidas">
          <q-card class="q-mb-sm">
            <div class="row">
              <div class="col-xs-12 col-md-2">
                <q-img
                  src="https://a-static.mlcdn.com.br/618x463/cerveja-heineken-puro-malte-lager-premium-long-neck-6-garrafas-de-330ml/magazineluiza/225339400/38384e0e6dacd5b1818bff0ff973c0a0.jpg"
                  :ratio="4 / 3"
                />
              </div>
              <div class="col-xs-12 col-md-10">
                <q-card-section>
                  <div class="row">
                    <div class="col-xs-11">
                      <div class="text-h6">Cerveja Heineken (long neck)</div>
                      <div class="text-subtitle2">R$ 8,00</div>
                    </div>
                    <div class="col-xs-1">
                      <div class="row justify-center">
                        <span class="quantity-label">0</span>
                      </div>
                      <div class="row justify-center">
                        <q-btn
                          round
                          size="xs"
                          color="primary"
                          icon="chevron_left"
                          class="q-mr-sm"
                        />
                        <q-btn
                          round
                          size="xs"
                          color="primary"
                          icon="chevron_right"
                        />
                      </div>
                    </div>
                  </div>
                </q-card-section>
                <q-card-section>
                  Lorem ipsum dolor sit amet, consectetur adipiscing elit
                </q-card-section>
              </div>
            </div>
          </q-card>
          <q-card class="q-mb-sm">
            <div class="row">
              <div class="col-xs-12 col-md-2">
                <q-img
                  src="data:image/jpeg;base64,"
                  :ratio="4 / 3"
                />
              </div>
              <div class="col-xs-12 col-md-10">
                <q-card-section>
                  <div class="row">
                    <div class="col-xs-11">
                      <div class="text-h6">Coca-cola (lata)</div>
                      <div class="text-subtitle2">R$ 4,00</div>
                    </div>
                    <div class="col-xs-1">
                      <div class="row justify-center">
                        <span class="quantity-label">0</span>
                      </div>
                      <div class="row justify-center">
                        <q-btn
                          round
                          size="xs"
                          color="primary"
                          icon="chevron_left"
                          class="q-mr-sm"
                        />
                        <q-btn
                          round
                          size="xs"
                          color="primary"
                          icon="chevron_right"
                        />
                      </div>
                    </div>
                  </div>
                </q-card-section>
                <q-card-section>
                  Lorem ipsum dolor sit amet, consectetur adipiscing elit
                </q-card-section>
              </div>
            </div>
          </q-card>
        </q-tab-panel>
      </q-tab-panels>
    </div>
  </div>
</template>

<script>
export default {
  data: () => ({
    tab: "lanches"
  })
};
</script>

<style lang="css">
.quantity-label {
  font-size: 35px;
  font-weight: bolder;
}
</style>
